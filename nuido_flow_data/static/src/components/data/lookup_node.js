...
export class LookupNode extends Node {
    ...
    onLookupFieldUpdate(path, info) {
        this.props.node.lookup_field = info.fieldDef ? info.fieldDef.name : "";
        this.refreshEdges();
    }
    onValueFieldUpdate(path, info) {
        this.props.node.value_field = info.fieldDef ? info.fieldDef.name : "";
        this.refreshEdges();
    }
    get modelSelectorId() {
...
LookupNode.template = "nuido_flow_data.lookup-node";
LookupNode.components = {
    ...Node.components,
    ModelSelectorEx,
