            ...
            if len(histories) > 0:
                ...
                for history in histories:
                    ...
                    for agent in self.groupchat.agents:
                        if agent == speaker:
                            self._append_oai_message(history, "user", agent, False) # from chat manager perspective speaker is user
                            agent._append_oai_message(history, "assistant", self, True) # from agent perspective itself is an assistant
                        else:
                            self._append_oai_message(history, "assistant", agent, True)
                            agent._append_oai_message(history, "user", self, False)

                for msg in fmsg:
                    self._append_oai_message(msg, "user", sender, False)
                    sender._append_oai_message(msg, "assistant", self, True)
