...
export function makeRoot(target) {
    ...
    root.classList.add("o-ai-portal-root");
    root.setAttribute("id", `o-ai-portal-root-${luxon.DateTime.now().toMillis() + Math.random()}`);
...
export async function makeShadow(root) {
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = url("/ai_portal_chat/assets.css");
    ...
    await Promise.all([
        stylesLoadedPromise,
        loadFont("FontAwesome", url("/ai_portal_chat/font-awesome")),
        loadFont("odoo_ui_icons", url("/ai_portal_chat/odoo_ui_icons")),
    ]);
...
/*
 * comes from make_awaitable_dialog.js
...
export function makeAwaitableAiCorner(aiCornerService, props, options) {
    return new Promise((resolve) => {
        aiCornerService.openAiCorner(
            props,
            {
                ...options,
                onClose: () => resolve(),
