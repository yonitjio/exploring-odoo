...
import { isOverlap, useMouseListener } from "@nuido/utils/owl_utils";
...
export class NuidoUi extends Component {
    ...
    setup() {
        ...
        useBus(this.state.env.nbus, this.env.channel + "/zoom_reset", this.zoom_reset.bind(this));
    }
    get documents() {
    ...
    deleteSelected() {
        this.state.env.nbus.trigger(this.state.env.channel + "/delete");
    }
    reset() {
        this.state.env.nbus.trigger(this.state.env.channel + "/reset");
    }
    clearSelection() {
        this.state.env.nbus.trigger(this.state.env.channel + "/clear");
    }
    updateEdgeType() {
    ...
    _toggleSelections(elements, selectionType) {
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            if (isOverlap(left, top, width, height, rect.left, rect.top, rect.width, rect.height)) {
                this.state.env.nbus.trigger(this.env.channel + "/select", {
                    id: elements[i].id,
                    type: selectionType
                });
            }
            else {
                this.state.env.nbus.trigger(this.env.channel + "/unselect", {
                    id: elements[i].id,
                    type: selectionType
                });
            }
    ...
    onMouseMove(event) {
        if (event.buttons == 2) {
            ...
            if (this.selectionState.selecting) {
                ...
                this._toggleSelections(nodeEls, "node");
                this._toggleSelections(pathEls, "edge");
                this._toggleSelections(jointEls, "joint");
            }
        }
    }
    onMouseUp(event) {
    ...
    change_translation() {
        ...
        doc.style.transform =
            "translate(" +
                ...
                this.state.env.ui.zoom +
                ")";
        this.state.env.nbus.trigger(this.state.env.channel + "/translation_changed", {
