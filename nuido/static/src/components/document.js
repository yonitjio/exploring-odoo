...
export class Document extends Component {
    ...
    setup() {
        ...
        useBus(this.env.nbus, this.env.channel + "/new", this.onNewNode.bind(this));
        useBus(this.env.nbus, this.env.channel + NodeMovedEventType, this.onNodeMoved.bind(this));
        useBus(this.env.nbus, this.env.channel + "/delete", this.onDeleteSelected.bind(this));
        useBus(this.env.nbus, this.env.channel + "/reset", this.onReset.bind(this));
        useBus(this.env.nbus, this.env.channel + EdgeTypeEventType, this.onEdgeTypeChanged.bind(this));
        useBus(this.env.nbus, this.env.channel + "/toggle", this.onToggleSelection.bind(this));
        useBus(this.env.nbus, this.env.channel + "/select", this.onSelect.bind(this));
        useBus(this.env.nbus, this.env.channel + "/unselect", this.onUnselect.bind(this));
        useBus(this.env.nbus, this.env.channel + "/clear", this.onClearSelected.bind(this));
        useBus(this.env.nbus, this.env.channel + "/edge-start", this.onStartConnect.bind(this));
        useBus(this.env.nbus, this.env.channel + "/edge-end", this.onEndConnect.bind(this));
    ...
    onNodeMoved(event) {
        ...
        if (doc.selected.findIndex(o => o.id === event.detail.id) > -1) {
            const selectedNodes = doc.selected.filter(o => (o.id !== event.detail.id) && (o.type === "node"));
            for (let i = 0; i < selectedNodes.length; i++) {
                const node = doc.nodes.find(o => o.id === selectedNodes[i].id);
                node.move(event.detail.x, event.detail.y);
            }
            const selectedJoints = doc.selected.filter(o => (o.id !== event.detail.id) && (o.type === "joint"));
