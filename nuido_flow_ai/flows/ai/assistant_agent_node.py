...
from .tools import get_chat_completion_client_node, get_tool_nodes, get_mcp_node, get_memory_nodes
...
class AssistantAgentNode(BaseNode):
    ...
    def _setup_assistant_agent(self):
        ...
        self.memory_nodes = get_memory_nodes(self)
        self.memories = []
        for m in self.memory_nodes:
            res = m.process({})
            self.memories.append(res["memory"])
    ...
    def _create_task(self, prompt):
        ...
        if isinstance(prompt, str):
            ...
            text = prompt["text"]
            if "attachments" in prompt and prompt["attachments"] is not None:
                #only images are supported, for now we only take the first image
                base64_image = prompt["attachments"][0]
                image = Image.from_base64(base64_image["data"])
    ...
    async def _ask_ai(self, params):
        ...
        try:
            ...
            try:
                prompt = ast.literal_eval(prompt_def)
            except:
                _logger.debug(f"Unable to load prompt with ast, trying with json: {prompt_def}")
                try:
                    prompt = json.loads(prompt_def)
                except:
                    _logger.debug(f"Unable to load prompt with json, it will be treated as regular string: {prompt_def}")
                    prompt = prompt_def
