...
from ..flows.ai.list_memory_node import ListMemoryNode
...
# MEMORY NODES
# List
def build_list_memory_node(node, edges):
    info = getDefaultInfo(node, edges)
    info["memories"] = node["memories"]

    return info

def create_list_memory_node(environment, create_function_registry, definitions, definition):
    return ListMemoryNode(environment, create_function_registry, definitions, definition)
