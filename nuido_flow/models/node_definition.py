...
class NodeDefinition(models.Model):
    ...
    def run(self, params):
        ...
        node_def = next((o for o in definitions if o["type"] == "StartNode"), None)
        if node_def is not None:
            monitor_process = self.env['ir.config_parameter'].sudo().get_param("nuido_flow.monitor_process", False)
            ...
            res = self.with_context(run_params=params, start_params=parametersJson)._run(definitions, node_def, params)
