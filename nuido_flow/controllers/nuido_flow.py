...
class NuidoFlowController(http.Controller):
    '''Controller for running Nuido flow definitions. '''

    @http.route('/nuidoflow/run', type='jsonrpc', auth='user', website=True)
    def run(self, def_id):
        '''
    ...
    @http.route('/nuidoflow/process', type='jsonrpc', auth='user', website=True)
