...
export class NuidoStudio extends Component {
    static res_model = "nuido_base.node.definition";
    static template = "nuido_base.studio";
    static components = { NuidoUi, SidebarMenu };
    static props = {
    ...
    setup() {
        ...
        useBus(this.state.nbus, this.channel + "/translation_changed", this.translation_changed.bind(this));
    ...
    async onWillStart() {
        ...
        if (activeId) {
            ...
            if (rec.length > 0) {
                ...
                this.currentDoc.import(rec[0]["raw"]);
    ...
    notifyNewNode(icon, title, type, x, y) {
        this.state.nbus.trigger(this.channel + "/new", { icon: icon, title: title, type: type, x: x, y: y });
    }
    onShowJson() {
        const jsonDoc = this.state.documents[0].export();
    ...
    async onSave() {
        const activeId = this.props.action.context.active_id;
        if (activeId) {
            const jsonDoc = this.currentDoc.export();
    ...
    deleteSelected() {
        this.state.nbus.trigger(this.channel + "/delete");
    }
    clearSelection() {
        this.state.nbus.trigger(this.channel + "/clear");
    }
    zoom_reset() {
        this.state.nbus.trigger(this.channel + "/zoom_reset");
    }
