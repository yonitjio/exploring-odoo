...
class AssistantAgentNode(AANode):
    def _setup_assistant_agent(self):
        ...
        if user.is_public:
            domain.append(("token", "=", context["run_params"]["token"]))

        chat_state_count = self.env["nuido_flow_ai_chat.chat.state"].search_count(domain)
        ...
        if self.definition["is_stateful"]:
            if chat_state_count == 0:
                self.env["nuido_flow_ai_chat.chat.state"].create({
                    ...
                    "node_def_id": node_def_id,
                    "node_id": self.id,
                    "token": context["run_params"]["token"] if user.is_public else ""
                })
        else:
            if chat_state_count > 0:
                recs = self.env["nuido_flow_ai_chat.chat.state"].search(domain)
                for rec in recs:
                    rec.unlink()
    ...
    async def _load_chat_state(self, context):
        if self.is_stateful and self.chat_mode is not None:
            ...
            self.chat_state = user.nuido_flow_ai_chat_states.search(domain)[0]

            state_key = f"{self.chat_mode}_state"
            if self.chat_state[state_key] != "":
                saved_state = json.loads(self.chat_state[state_key])
    ...
    async def _save_chat_state(self):
        if self.is_stateful and self.chat_mode is not None and self.chat_state is not None:
            ...
            state_key = f"{self.chat_mode}_state"
            self.chat_state[state_key] = jsonState
