...
export class ChatDialog extends AiChatContainer {
    static template = "nuido_flow_ai_chat.chat-dialog";
    static components = {
    ...
    reset() {
        this.chat.testChatReset(this.props.nodeDefId, this.props.token);
        super.reset();
    }
    get isProcessing() {
    ...
    onSendMessage(message, history) {
        const res = this.chat.testChat(this.props.nodeDefId, this.props.token, this.props.channel, message);
        if (!res) {
